...
from geogig.ui.mergedialog import Ui_MergeDialog
...
class MergeDialog(QtGui.QDialog):
    ...
    def treeItemExpanded(self, item):
        ...
        if item.childCount() > 0:
            return          
        headDiffsFeaturePaths = set(d.path for d in self.repo.diff(item.ancestor, geogig.HEAD, item.path))
        toMergeDiffsFeaturePaths = set(d.path for d in self.repo.diff(item.toMerge, geogig.HEAD, item.path))
    ...
    def branchToMergeChanged(self):
        ...
        ancestor = self.repo.commonancestor(geogig.HEAD, toMerge).ref
        diffsHead = self.repo.difftreestats(geogig.HEAD, ancestor)
        diffsToMerge = self.repo.difftreestats(toMerge, ancestor)
        toMergeItem = QtGui.QTreeWidgetItem(["Layers changed only in branch to merge"])
        self.ui.changesTree.addTopLevelItem(toMergeItem)
        headItem = QtGui.QTreeWidgetItem(["Layers changed only in current branch"])
        self.ui.changesTree.addTopLevelItem(headItem)
        bothBranchesItem = QtGui.QTreeWidgetItem(["Layers changed in both branches"])
    ...
    def updateCurrentPath(self, path, toMerge, ancestor):
        self.cleanCanvas()
        versions = [Feature(self.repo, toMerge, path), Feature(self.repo, geogig.HEAD, path), 
    ...
    def showFeatureAttributes(self, versions):         
        ...
        for idx, name in enumerate(allAttribs):
            ...
            if origingeom is None and isinstance(values[0], Geometry):                                        
                origingeom =values[0]                 
            if oursgeom is None and isinstance(values[1], Geometry):                                        
                oursgeom = values[1]
            if theirsgeom is None and isinstance(values[2], Geometry):                                        
                theirsgeom = values[2]
    ...
    def createLayers(self, origingeom, oursgeom, theirsgeom):               
        ...
        if origingeom is not None:
            qgsgeom = QgsGeometry.fromWkt(origingeom.geom)
            geomtype = types[int(qgsgeom.type())][0]            
            if origingeom.crs is not None:
            ...
            self.originLayer = QgsVectorLayer(geomtype + "?crs=EPSG:4326", "origin", "memory")
        ...
        if oursgeom is not None:
            qgsgeom = QgsGeometry.fromWkt(oursgeom.geom)
            geomtype = types[int(qgsgeom.type())][0]            
            if oursgeom.crs is not None:
                targetCrs = self.mapCanvas.mapRenderer().destinationCrs()
                crsTransform = QgsCoordinateTransform(QgsCoordinateReferenceSystem(oursgeom.crs), targetCrs) 
                qgsgeom.transform(crsTransform)                            
            style = types[int(qgsgeom.type())][2]                     
            self.oursLayer = QgsVectorLayer(geomtype + "?crs=EPSG:4326", "ours", "memory")
        ...
        if theirsgeom is not None:
            qgsgeom = QgsGeometry.fromWkt(theirsgeom.geom)
            geomtype = types[int(qgsgeom.type())][0]            
            if theirsgeom.crs is not None:
                targetCrs = self.mapCanvas.mapRenderer().destinationCrs()
                crsTransform = QgsCoordinateTransform(QgsCoordinateReferenceSystem(theirsgeom.crs), targetCrs) 
                qgsgeom.transform(crsTransform)            
            style = types[int(qgsgeom.type())][3]                     
            self.theirsLayer = QgsVectorLayer(geomtype + "?crs=EPSG:4326", "theirs", "memory")
