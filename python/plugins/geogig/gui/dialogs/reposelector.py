...
from geogig.ui.reposelector import Ui_RepoSelectorDialog 
...
from geogig.gui.dialogs.addrepodialog import AddRepoDialog 
...
from geogig.tools.ujoconnector import getUjoRepositories,\
    AuthenticationException, createUjoRepository
from geogig import config
from geogig.tools.repodecorator import LocalRepository, InvalidRepoException,\
    wrongRepoIcon
from createujorepodialog import CreateUjoRepoDialog
from requests.exceptions import RequestException
from geogig.gui.executor import execute
import webbrowser
from geogig.tools import ujoconnector
from simpleimportdialog import SimpleImportDialog


deleteIcon = QtGui.QIcon(os.path.join(os.path.dirname(__file__), 
                            os.pardir, os.pardir, "ui", "resources", "delete.gif"))
refreshIcon = QtGui.QIcon(os.path.join(os.path.dirname(__file__), 
                            os.pardir, os.pardir, "ui", "resources", "refresh.png"))
...
class RepoSelectorDialog(QtGui.QDialog):
    ...
    def showTreePopupMenu(self, point):                                    
        ...
        if toDelete:                    
            ...
            menu.exec_(point)
        if len(items) == 1 and items[0] == self.ujoReposItem and self.user is not None:            
            menu = QtGui.QMenu()
            refreshAction = QtGui.QAction(refreshIcon, "Refresh", None) 
            def _update():                  
                self.loadUjoRepos(self.user, self.password)                             
                self.updateUjoReposItem()
            refreshAction.triggered.connect(_update)                
    ...
    def descriptionLinkClicked(self, url):        
        ...
        if url == "upload":
            ...
            if repo is not None:            
                ...
                self.currentRepo.setUjoData(repo.url, repo.title, repo.description)
    ...
    def loginLinkClicked(self, url):
        if url == "site":                                
            ...
            dlg = UserPasswordDialog(title = "Ujo credentials")
            dlg.exec_()
    ...
    def openOrCloneRepo(self):
        ...
        if isinstance(self.currentRepo, LocalRepository): 
            ...
            if cloneDirectly and os.path.exists(path):
                ...
                folder = unicode(QtGui.QFileDialog.getExistingDirectory(self, "Copied repository folder", path))        
            if folder != "":
                ujoUrl = self.currentRepo.url
                ujoDescription = self.currentRepo.description
                ujoTitle = self.currentRepo.title
                if self._addRepo(self.currentRepo.url, folder, (ujoUrl, ujoTitle, ujoDescription), 
    ...
    def saveLocalReposToSettings(self):
        ...
        QtCore.QSettings().setValue("/GeoGig/Repos", value)
        
    def readLocalReposFromSettings(self): 
        repos = unicode(QtCore.QSettings().value("/GeoGig/Repos", ""))
        ...
        if dlg.ok:
            if dlg.mode == AddRepoDialog.ADD:
                ...
                if dlg.importData and created:
                    importDlg = SimpleImportDialog(self, self.currentRepo.repo)                        
                    importDlg.exec_()                      
            self.saveLocalReposToSettings()
    ...
    def _addRepo(self, url, clonePath = None, ujoData = None, user = None, password = None):                                                            
        try:                                                  
            ...
            QtGui.QMessageBox.warning(self, 'Error running geogig',
    ...
    def updateLogLabel(self):        
        if self.user is not None:
            self.ui.loggedAsLabel.setText("Logged in as %s. &nbsp; <a href='login'>Change</a> &nbsp; <a href='site'> Open account page</a>" %(self.user))            
        else:
        ...
        if repo is None:
            ...
            try:
                ...
                if isinstance(repo, LocalRepository) or repo.localClone is not None:
                    ...
                    self.ui.openOrCloneRepoButton.setText("Copy repository")                                                    
...
def getUjoCredentials():
    global _repoSelectorDialog
    if _repoSelectorDialog is None:
        return None
    
    credentials = (_repoSelectorDialog.user, _repoSelectorDialog.password)
