...
from geogig.ui.addrepodialog import Ui_AddRepoDialog
...
from geogigpy.repo import isremoteurl
...
class AddRepoDialog(QtGui.QDialog):
    ...
    def selectRepoFolder(self):
        ...
        path = settings.value("/GeoGig/LastAddedRepo", "") 
        folder = unicode(QtGui.QFileDialog.getExistingDirectory(self, "Ujo repository folder", path))       
        if folder != "":            
            self.ui.addRepoFolderBox.setText(folder)
                    
    def selectCloneFolder(self):
        folder = unicode(QtGui.QFileDialog.getExistingDirectory(self, "Ujo repository folder"))        
        if folder != "":
        ...
        if self.ui.cloneCheckBox.isChecked():
            ...
            if isremoteurl(self.path):
                QtGui.QMessageBox.warning(self, "Remote repositories not supported",
                        ...
                        "Select the 'Copy repo' check box to copy it locally", 
            ...
            self.clonePath = None
        settings = QtCore.QSettings()
        settings.setValue("/GeoGig/LastAddedRepo", self.path)            
        self.close()
    ...
    def createRepo(self):        
        ...
        if isremoteurl(self.path):
            ...
            try:
                execute(lambda : GeoGigServerConnector.createrepo(url, name))
            ...
            ok = QtGui.QMessageBox.warning(self, "Create remote repository",
                    ...
                    "To be able to connect to it, you must copy it locally\n"
