...
from geogig.tools.utils import mkdir
import requests
...
repoIcon = QtGui.QIcon(os.path.join(os.path.dirname(__file__), 
                            os.pardir, "ui", "resources", "geogig-logo-24.png"))
...
class LocalRepository(object):
    ...
    def __init__(self, path):
        ...
        self.description = ""
        self.ujoUrl = None
        filename = os.path.join(self.url, "geogigpy", "ujo")
    ...
    def setUjoData(self, ujoUrl, ujoTitle, ujoDescription):
        self.name = os.path.basename(ujoUrl)
        self.title = ujoTitle
        self.description = ujoDescription
        self.ujoUrl = ujoUrl
        mkdir(os.path.join(self.url, "geogigpy"))
        filename = os.path.join(self.url, "geogigpy", "ujo")
    ...
    def writeLayersFile(self):
        ...
        mkdir(os.path.join(self.url, "geogigpy"))
        filename = os.path.join(self.url, "geogigpy", "layers")
        with open(filename, "w") as f:
    ...
    @property
    def icon(self):
        if os.path.exists(os.path.join(self.url, ".geogig")):
            return repoIcon
        else:
    ...
    def fullDescription(self):
        if not os.path.exists(os.path.join(self.url, ".geogig")):
...
class UjoRepository(object):
    ...
    def fullDescription(self):        
        ...
        if self.localClone is not None:
            s += "<p><b>COPIED LOCALLY AT: </b>%s</p>" % (self.localClone)
