...
class GeoDB:
    ...
    def get_primary_key_column(self, table):            
        ...
        sql = """SELECT               
                ...
                AND indisprimary""" % (self._quote_str(table))
    ...
    def table_add_serial_column(self, schema, table, fieldname):
        table_name = self._table_name(schema, table)
        try:
            self.table_delete_column(table, fieldname, schema)
        except DbError,e:            
            pass        
        sql = ("DROP SEQUENCE %s_%s_seq" % (table, fieldname))
        try:
            self._exec_sql_and_commit(sql)            
        except DbError,e:            
            pass      
        sql = "CREATE SEQUENCE %s_%s_seq INCREMENT 1 START 1" % (table, fieldname)
        self._exec_sql_and_commit(sql)  
        sql = ("ALTER TABLE %s ADD %s bigint DEFAULT nextval('%s_%s_seq')" 
                % (table_name, fieldname, table, fieldname))
        self._exec_sql_and_commit(sql)
